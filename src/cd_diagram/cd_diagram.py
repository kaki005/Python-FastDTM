...
def graph_ranks(
    ...
    **kwargs,
    ...
    for i in range(math.ceil(k / 2), k):
        ...
        line(
            [(rankpos(ssums[i]), cline), (rankpos(ssums[i]), chei), (textspace + scalewidth + 0.1, chei)],
            linewidth=linewidth,
        )
        if labels:
    ...
    for clq in cliques:
        ...
        if min_idx >= len(nnames) / 2 and achieved_half == False:
            ...
            achieved_half = True
        line(
            [(rankpos(ssums[min_idx]) - side, start), (rankpos(ssums[max_idx]) + side, start)], linewidth=linewidth_sign
        )
        start += height
...
def wilcoxon_holm(alpha=0.05, df_perf=None):
    ...
    df_ranks = pd.DataFrame(
        data=rank_data, index=np.sort(classifiers), columns=np.unique(sorted_df_perf["dataset_name"])
    )
